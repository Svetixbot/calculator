...
object ResultSpec extends Properties("Result") {
  ...
  //   forAll(resultGen) { result =>
		// result match {
		//     case Fail(_) => result.fold(_ => 0, x => x) == 0
		//     case Ok(a) => result.fold(_ => 0, x => x) == a
		//   }
  //   }
  ...
  //   forAll(resultGen) { result =>
		// result match {
		//     case Fail(e) => result.flatMap(_ => Ok(1)) == Fail(e)
		//     case Ok(a) => result.flatMap(_ => Ok(1)) == Ok(1) && 
